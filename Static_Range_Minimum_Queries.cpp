...
class SS{
    ...
    void build(vll &arr,ll ind,ll left,ll right) {
        if(left == right) {
            seg[ind] = arr[left];
            return;
        }
        ll mid = left+(right-left)/2;
        build(arr,2*ind+1,left,mid);
        build(arr,2*ind+2,mid+1,right);
    ...
    ll query(ll l,ll r,ll ql,ll qr,ll ind) {
        ...
        ll mid = l+(r-l)/2;
        ll ls = query(l,mid,ql,qr,2*ind+1);
        ll rs = query(mid+1,r,ql,qr,2*ind+2);
        return min(ls,rs);
    }
};
