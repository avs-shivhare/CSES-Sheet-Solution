...
int main() {
    ...
    // wh(t--) {
        ...
        cin>>n>>m>>k;
        vll apart,applicant;
        in<vll,ll,ll>(applicant,n);
        in<vll,ll,ll>(apart,m);
        sort(all(apart));
        sort(all(applicant));
        ll i =0,j =0;
        ll ans = 0;
        wh(i<n && j<m) {
            if(apart[j]-k <= applicant[i] && applicant[i] <= apart[j]+k) {
                ans++;
                //outall<vll>({apart[j]-k,applicant[i],apart[j]+k,k});
                i++;
                j++;
            }
            else if(applicant[i] < apart[j]) i++;
            else j++;
        }
        out(ans);
    // }
