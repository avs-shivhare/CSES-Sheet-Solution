...
class SS{
    ...
    void update(ll l,ll r,ll ql,ll qr,ll val,ll ind) {
        if(l > qr || ql > r || l>r) return;
        if(lazy[ind] != 0) {
        ...
        ll mid = l+(r-l)/2;
        update(l,mid,ql,qr,val,2*ind+1);
        update(mid+1,r,ql,qr,val,2*ind+2);
        seg[ind] += seg[2*ind+1]+seg[2*ind+2];
        return;
    }
    ll query(ll l,ll r,ll idx,ll ind) {
        if(lazy[ind] != 0) {
            seg[ind] += (r-l+1)*lazy[ind];
            if(l != r) {
                lazy[2*ind+1] += lazy[ind];
        ...
        ll mid = l+(r-l)/2;
        if(idx <= mid) {
            return query(l,mid,idx,2*ind+1);
        } 
        return query(mid+1,r,idx,2*ind+2);
    }
};
    ...
    // wh(t--) {
        ...
        wh(m--) {
            ...
            if(type == 1) {
                ...
                l--;
                r--;
                s.update(0,n-1,l,r,val,0);
            }
            else {
                ll idx;
                cin>>idx;
                idx--;
                ll ans =s.query(0,n-1,idx,0);
                out(ans);
            }
        }
    // }
