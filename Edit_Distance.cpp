...
ll find(ll i,ll j,str &s,str &t) {
    if(j <0 && i<0) return 0;
    if(j < 0) return i+1;
    if(i<0) return j+1;
    if(dp[i][j] != -1) return dp[i][j];
    ll take = 1e9,notTake = 1e9;
    if(s[i] == t[j]) take = 0+find(i-1,j-1,s,t);
    notTake = min({1+find(i-1,j,s,t),1+find(i-1,j-1,s,t),1+find(i,j-1,s,t)});
