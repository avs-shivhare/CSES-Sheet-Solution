...
void dfs(ll row,ll col,vvb &vis,vvc &arr,ll n,ll m) {
    vis[row][col] = true;
    f(i,0,4) {
        ll newrow = row+rowdiff[i];
        ll newcol = col+coldiff[i];
        if(newrow >= 0 && newrow < n && newcol >= 0 && newcol<m && arr[newrow][newcol] == '.' && !vis[newrow][newcol]) {
